				 ...
				 int which_part, disk_partition_t *info)
{
	...
	int i;
     ...
     PRINTF("INSIDE GET_PARTITION_INFO_EXTENDED 1\n");
	...
	if(pt->boot_ind != 0x80 && pt->boot_ind != 0x0){
     ...
     PRINTF("INSIDE GET_PARTITION_INFO_EXTENDED 2\n");
	...
	for (i = 0; i < 4; i++, pt++) {
			...
			switch(dev_desc->if_type) {
				...
				case IF_TYPE_ATAPI:
					sprintf (info->name, "hd%c%d\n", 'a' + dev_desc->dev, part_num);
					 ...
					 PRINTF("INSIDE GET_PARTITION_INFO_EXTENDED 2.1 %s \n", info->name);

					break;
				case IF_TYPE_SCSI:
					sprintf (info->name, "sd%c%d\n", 'a' + dev_desc->dev, part_num);

					 PRINTF("INSIDE GET_PARTITION_INFO_EXTENDED 2.2 %s \n", info->name);

					break;
				case IF_TYPE_USB:
					//sprintf (info->name, "usbd%c%d\n", 'a' + dev_desc->dev, part_num);
                    sprintf(info->name, "usbd%c%d", 'a', part_num);

			         PRINTF("INSIDE GET_PARTITION_INFO_EXTENDED 2.3 >%s< \n", info->name);
			         PRINTF("INSIDE GET_PARTITION_INFO_EXTENDED 2.3.0 >%d< \n", strlen(info->name));
			         PRINTF("INSIDE GET_PARTITION_INFO_EXTENDED 2.3.1 >%s< \n", dev_desc->dev);
			         PRINTF("INSIDE GET_PARTITION_INFO_EXTENDED 2.3.2 >%d< \n", part_num);

					break;
				case IF_TYPE_DOC:
					sprintf (info->name, "docd%c%d\n", 'a' + dev_desc->dev, part_num);

				     PRINTF("INSIDE GET_PARTITION_INFO_EXTENDED 2.4 %s \n", info->name);

					break;
				default:
					sprintf (info->name, "xx%c%d\n", 'a' + dev_desc->dev, part_num);

					 PRINTF("INSIDE GET_PARTITION_INFO_EXTENDED 2.5 %s \n", info->name);
     ...
     PRINTF("INSIDE GET_PARTITION_INFO_EXTENDED 3\n");
	...
	for (i = 0; i < 4; i++, pt++) {
		if (is_extended (pt->sys_ind)) {
			int lba_start = le32_to_int (pt->start4) + relative;
               ...
               PRINTF("INSIDE GET_PARTITION_INFO_EXTENDED 4\n");
