...
//try alternative usb init method? youlian@onion.io
...
#define	CONFIG_USB_MAX_CONTROLLER_COUNT 1
        for (i = 0; i < CONFIG_USB_MAX_CONTROLLER_COUNT; i++) {
                ...
                /* init low_level USB */
                USB_PRINTF("LOW LEVEL INIT USB%d:   \n", i);
			...
			void *data, unsigned short size, int timeout)
{
	...
	if(dev->status==0)
	{
	    USB_PRINTF("usb_control_msg: status = success?\n");

		return dev->act_len;
	}
	else
	{
		USB_PRINTF("USB: usb_control_msg: status not success?\n");
...
int usb_parse_config(struct usb_device *dev, unsigned char *buffer, int cfgno)
{
	...
	dev->config.desc.wTotalLength=swap_16(dev->config.desc.wTotalLength);
...
int usb_new_device(struct usb_device *dev)
{
	int addr, err;
	int tmp;
	ALLOC_CACHE_ALIGN_BUFFER(unsigned char, tmpbuf_buf, USB_BUFSIZ);
	unsigned char *tmpbuf = KSEG1ADDR(tmpbuf_buf);
