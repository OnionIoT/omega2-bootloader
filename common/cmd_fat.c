...
int do_fat_fsload (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	if (dev_desc==NULL) {
		puts ("\nCMD_FAT: ** Invalid boot device **\n");
		return 1;
	}

	if (*ep) {
		if (*ep != ':') {
			puts ("\nCMD_FAT: ** Invalid boot device, use `dev[:part]' **\n");
			return 1;
		}
		part = (int)simple_strtoul(++ep, NULL, 16);
	}

    int fat_reg = fat_register_device(dev_desc,part);

	if (fat_reg!=0) {
		printf ("\nCMD_FAT: ** Unable to use %s %d:%d for fatload **\n",argv[1],dev,part);
		return 1;
	}
	...
	if(size==-1) {
		printf("\n FAT COMMAND: Unable to read file \"%s\" from device %s %d:%d \n",argv[4],argv[1],dev,part);
		return 1;
	}

	printf ("FAT: %ld Bytes read\n", size);
