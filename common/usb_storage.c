...
int usb_stor_scan(int mode)
{
	...
	for(i=0;i<USB_MAX_DEVICE;i++) {
		...
		{ /* ok, it is a storage devices */
		    ...
		    printf("Identifying a storage device...");
	...
	printf("USB_STORAGE: %d Storage Device(s) found\n", usb_max_devs);
...
static int usb_stor_BBB_reset(struct us_data *us)
{
	...
	if((result < 0) && (us->pusb_dev->status & USB_ST_STALLED))
	{
		...
		printf("BBB reset stall YES");

		return -1;
	}
	else
	{
	    printf("BBB reset stall NO");
	}
...
int usb_stor_BBB_clear_endpt_stall(struct us_data *us, __u8 endpt)
{
    ...
    printf("INSIDE usb_stor_BBB_clear_endpt_stall");
...
retry_it:
        ...
        //printf ("USB_MAX_READ_BLK: %d", USB_MAX_READ_BLK);

#define PROGRESS_SCALE 100

        if(smallblks==USB_MAX_READ_BLK)
        {
            if (total_blocks<PROGRESS_SCALE)
		    {
			    usb_show_progress();
		    }
		    else
		    {
		        if (total_blocks % PROGRESS_SCALE == 0)
		        {
		            usb_show_progress();
		        }
		    }
